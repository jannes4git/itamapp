<template>
    <div v-if="assetLoaded" class="detailContent">
        ...
        <button @click="deleteAsset()">Löschen</button>
...
export default {
    ...
    computed: {
        ...
        customFieldValuesFetch() {
            return this.$store.getters.getCustomFieldValuesById(
        ...
        inventar() {
            return this.$store.getters.getInventarById(
                Number(this.$route.params.id)
            );
        },
        personen() {
    ...
    beforeMount() {
        ...
        this.customFields.forEach((field) => {
            let value = this.customFieldValuesFetch.find(
